...
// /**
//     * @typedef {import('../libs/TypeDef').AppiumServer} TAppiumServer
//  */

// /**
//  * @type {Map<String, TAppiumServer>} 
//  */
let appiumServerList: Map<String, TAppiumServer> 

const totalServer = process.env.NODE_ENV == "development" ? 3 : 20
const ports = Array(totalServer).fill(0).map((val, idx) => idx + 20001)
...
class AppiumManager {
    ...
    async appiumOsPidsPorts() {
        ...
        let msg = await this.grepOsAppiumPidsText()
        ...
        let pidsPorts = msg.split("\n")
            ...
            .map(txt => {
                ...
                const textTemp = txt.replace(/\s\s+/gm, " ").split(" ")
    ...
    async grepOsAppiumPidsText() {
        ...
        let msg = await new Promise((resolve, reject) => {
            ...
            // exec('tasklist | findstr "Appium Server GUI.exe"', (error, stdout, stderr) => {
            //     if (error) reject(error)
        ...
        // const msg = "user01     20990   20908  0 13:45 ?        00:00:02 node /home/user01/.nvm/versions/node/v19.2.0/bin/appium server -pa /wd/hub -p 20001 --session-override"
    ...
    async assignServerToDeviceId(deviceId) {
        ...
        /** @type {AppiumServer} */
        let alreadyAssigned: TAppiumServer
        for (const [key, server] of appiumServerList) {
        ...
        /** @type {TAppiumServer} */
        let serverToBeAssign: TAppiumServer
        for (const [key, server] of appiumServerList) {
            if (server.deviceId == "") {
    ...
    async unAssignDeviceIdFromServer(deviceId) {
        ...
        /** @type {AppiumServer} */
        let serverToBeUnassign: TAppiumServer
        for (const [key, server] of appiumServerList) {
            if (server.deviceId == deviceId) {
